...
2. 執行 `npm run audio` 產生四組核心語音（美式與英式的 Kokoro 與 edge-tts）。
   已存在的檔案會自動跳過，所以重跑只會補上新課程，一週的故事約兩分鐘。
   其他口音是隨選的：想補的時候再跑 `npm run audio:accents`（六個口音）
   或 `npm run audio:all`（含 Chatterbox，很慢，約每句十秒）。
3. 執行 `npm run index` 產生含日期、系列與實際字數的摘要，以及音檔清單。
4. 執行 `npm run check`；它會檢查真實日期、字數、中文翻譯、故事四段、理解題、
   系列日序、連續日期、標題防重複，以及每一課的四組核心音檔是否齊全。
   隨選口音可以整組缺席，但不能只做一半。
