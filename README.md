...
複習有兩種卡。**辨識**先播音再顯示答案 —— 這是原本的做法,測的是聽得懂。
**回想**只給中文語意和英文開頭幾個字,要你先把整句說出來,才播原音對照 ——
測的是想不想得起來。前者是輸入,後者是輸出,而對話需要的是後者。
回想卡只給已經辨識成功過的句子(`js/recall.js`),沒聽懂過的句子先回到辨識。

複習預設**每次換一個聲音**(`js/voices.js` 的 `rotateVoice`)。同一句隔幾天回來時
由不同人唸,是 high-variability training 便宜的那一半 —— 音檔本來就在硬碟上,
而聽得懂一個人不等於聽得懂所有人。可在設定關閉。

## 功能

| | 說明 | 需要 API? |
|---|---|---|
| **聽力訓練** | 上面的七步循環,111 課 1874 句 | 否 |
| **課前字詞預習** | 每課先聽 4–6 個候選詞；人工課表優先，其餘本機自動挑選並可查英中／英英 | 內建詞免連線；其他詞按需查公開字典 |
| **每日課程** | 每日約 500 字原創故事,同一主題可連載 1–3 日 | 否 |
| **連續播放** | 完整課文或自訂任意多句,可調速度與句間停頓;按句旁 ▶ 播放該句 | 否 |
| **點字查詞** | 課文點字或選取片語,英中／英英切換、原句與生字收藏 | 內建／快取免連線;其他詞按需查公開字典,不用金鑰 |
| **真人錄音課程** | 60 課取自 VOA 公有領域錄音,附中文與逐句時間軸 | 否 |
| **多口音朗讀** | 美/英/澳/印/愛爾蘭/南非/加拿大/紐西蘭,可變速 | 否 |
| **免持模式** | 連續播放接續下一課,支援耳機與鎖定畫面控制、螢幕不休眠 | 否 |
| **聽的時候換聲音** | 訓練頁與播放頁隨時切口音、聲音、語速、中文,語速即時生效 | 否 |
| **跟讀評分** | 瀏覽器內建語音辨識,逐字標出漏掉的詞 | 否 |
| **間隔複習** | 兩種卡:辨識(先播再顯示)與**回想**(先自己說出來再聽) | 否 |
| **提取練習** | 中文語意 + 英文開頭當提示,靜默思考後開口,再對照原音 | 否 |
...
課程預覽、完整播放、精聽的「對答案」以後都可以點字查詞。拖選同一句內 1–6 個英文單字後,
按「查選取的字詞」可查片語。已知片語會優先整組顯示,也可切回只查點到的單字。
連續播放中查詞會暫停;關閉字卡後需手動續播。盲聽階段不提供查詞入口。

首版含 110 個自編詞條與部分課／句專用詞義,不是完整字典。只在按「查開放字典」時查詢 Wiktionary,
只傳送所選字詞;未收錄或缺中文時會明示。內建及成功快取詞可離線查看,外部查詢仍需網路。
英英解釋不代表已完成 CEFR 分級審校,一般詞條也不會自動判定本句詞義。

「設定 → 生字收藏」保留字詞、原句與來源；完整資料備份包含收藏、課程進度、自訂課程、錄音、聊天與影片資料，並可在另一個 Echo 裝置還原。API key 與離線音檔不會寫入備份。
收藏不會變更句子熟練度或自動加入 SRS。

### 課前預習與換條件任務

目前有 5 課人工編輯好的預習：`daily-2026-08-23`（L3，核心 6 詞＋選用 2 詞）、
`l1-05`「外帶午餐」、`l1-07`「想買的餐點沒有了」、`l2-07`「房間還沒準備好」、`l2-08`「這天不行，換一天好嗎？」（各 6 詞）。
其餘所有內建課與匯入課會在裝置上自動挑 4–6 個候選詞；畫面明示尚未逐詞人工編輯。已收錄詞直接顯示英中／英英，其他詞須自行按查詞確認，不會把占位字冒充翻譯，也不會上傳全文。
從課程詳情進入，先聽詞，再揭示本課中文義、簡易英英與不透露劇情的搭配。
每項分開記錄聽辨與文字理解的自評，可跳過、續練、重練不熟的詞；不影響句子 SRS。
暖身與任務發音使用裝置語音，不代表所有裝置離線時都已有英語聲音。

`l1-07` 是 12 回合原創 L1 延伸對話，銜接既有外帶午餐，練「售完時改點」。新加的兩個 L2 情境分別銜接飯店入住與週末計畫，練房間未好時確認入住時間，以及婉拒後協調另一個日期、時間與地點。
課後有 3 題中文理解題（選項打散、答後解釋），以及改換材料的固定角色卡。
任務可選「只有任務／關鍵字／句架」，再自行評估是否完成；不是 AI 對話，不會錄音或自動判斷溝通成功。
新紀錄跟收藏一樣包含在完整備份；還原會先驗證檔案，再用單一資料庫交易取代本機資料。另見 [SDD-002](docs/sdd/002-lesson-preparation.md)與 [SDD-003](docs/sdd/003-complete-curriculum-backup-release.md)。

## 課程內容

111 課、1874 句,難度 L1 到 L5 遞增。分成兩類:
...
每次施工遵循 **SDD → 施工節點紀錄 → 程式與測試 → know-how → 驗收**。
先讀 [工程索引](docs/engineering/README.md) 與 [工程規則](AGENTS.md);工程索引連結各功能規格、程式入口、測試與開發紀錄。
目前已建立 ENG-001 的完整追蹤,不代表既有功能的歷史規格已全部補齊。
...
查詞另有 [本機瀏覽器驗證頁](tools/qa/word-lookup.html),需從本機 HTTP 伺服器開啟。此頁不隨 Firebase 部署,
會驗證字典 HTML 抽取、同句選取、字卡焦點與非同步關閉。它不取代 iOS／Android 真機、斷網與語音驗收。
預習的 [資料庫驗證頁](tools/qa/learning.html) 另測原子寫入、錯誤回滾及分開保存學習成果；只用獨立暫存鍵，結束後清理。
...
從舊的 `echo-v10` 第一次升級時,舊頁面還沒有更新按鈕;看到舊版提示後,請把所有 Echo 分頁或 PWA 完全關閉再重開一次。新版會在沒有舊頁仍使用它時安全啟用;之後所有版本都會使用持續提示與「重新載入更新」按鈕。
